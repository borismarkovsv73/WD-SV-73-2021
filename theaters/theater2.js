...
let pozoristeURL = firebaseURL + "pozorista/-MNQg8Nd8YPRs-5Kbgqu.json"
let predstaveURL = firebaseURL + "predstave/-MNVEu6iMr2EFlQO6TW60.json"
    ...
    if (this.readyState == 4) { 
        if (this.status == 200) { 
            ...
            for (let i in predstave) {
                ...
                let template = `
                <li><a href="../plays/showA${a}.html"><i class="fa-solid fa-star"></i> ${predstava.naziv} <br><span class="extra_info">${predstava.kratakOpis}</span></a></li>
                `;
                target.innerHTML += template; 
                a += 1
            }
